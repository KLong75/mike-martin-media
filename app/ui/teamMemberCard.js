  ...
  return (
    ...
    <div
      ...
      <div className="absolute bottom-0 left-0 w-full h-1/8 z-50 bg-black bg-opacity-50 flex flex-col justify-center text-white p-4  group-hover:bg-opacity-100 transition-bg duration-2000 ease-in-ou ">
        ...
        <div className="relative mt-  w-full">
          <div className="mt-[1.5px] border-b border-2 border-white w-0 group-hover:w-full opacity-0 group-hover:opacity-100 transition-all duration-2000 ease-in-out"></div>
          <div className="absolute right-[-3px] top-0 h-full w-0 group-hover:w-4 opacity-0 group-hover:opacity-100 transition-all duration-3000 ease-in-out">
    ...
    <Dialog open={bioOpen} onClose={handleClose}>
      ...
      <DialogPanel className="fixed inset-0 flex justify-center items-center z-50">
        <div className="relative bg-black text-white p-6 px-12 max-w-2xl w-3/4 h-auto overflow-y-auto">
          ...
          <div className="flex flex-col items-start mt-6 md:p-2">
            <h2 className="text-2xl font-semibold">{name}</h2>
            <h3 className="text-xs uppercase font-semibold">{role}</h3>
            <p className="my-6 text-lg">{about}</p>
          </div>
        </div>
      </DialogPanel>
    </Dialog>
