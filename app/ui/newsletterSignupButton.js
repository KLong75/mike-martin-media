export default function NewsletterSignupButton() {
  const handleClick = () => {
    ...
    setTimeout(() => {
      const formWrapper = document.querySelector("._form-wrapper");
      if (formWrapper) {
        formWrapper.style.display = "block";
      }
    }, 200); // Wait for the form to be injected
  };

  return (
    <button
      id="newsletter-signup-button"
      className="bg-white text-black font-bold rounded-full px-2 py-1 mt-2 text-sm md:hover:scale-105 transition duration-500 ease-in-out"
      onClick={handleClick}>
